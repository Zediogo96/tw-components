...
const SkewedInfiniteScroll: FC = () => {
    ...
    const duplicatedItems = useMemo(() => {
        // Create duplicate items with unique keys for React
        const clonedItems = items.map(item => ({
            ...item,
            id: `clone-${item.id}`
        }));

        // Return the original items followed by their clones
                ...
                >
                    <div className="mx-auto grid h-[250px] w-[300px] animate-skew-scroll grid-cols-1 gap-5 sm:w-[600px] sm:grid-cols-2">
                        {duplicatedItems.map((item, index) => (
                            <div
                                key={item.id}
                                className={`flex min-h-15 cursor-pointer items-center space-x-2 rounded-md border border-gray-100 px-5 shadow-md transition-all hover:-translate-y-1 hover:translate-x-1 hover:scale-[1.025] hover:shadow-xl dark:border-gray-800 ${index % 2 ? 'translate-y-6' : ''}`}
                            >
                                <svg
